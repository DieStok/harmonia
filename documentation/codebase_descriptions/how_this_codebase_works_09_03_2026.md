...
**Date:** 09-03-2026 (updated from 05-03-2026)
...
- **Phoenix/OpenTelemetry tracing (Phase 1):**
  - New file: `src/automation/tracing.py` — OTel/Phoenix tracing module. Provides `init_tracing()` to configure TracerProvider with OTLP HTTP exporter, context managers for span creation (`experiment_span`, `turn_span`, `llm_call_span`, `tool_span`), and utility functions (`extract_usage_records`, `extract_code_executions`, `calculate_turn_cost`, `set_llm_usage`). All tracing is opt-in and gracefully degrades if Phoenix is unreachable or dependencies are missing.
  - New file: `scripts/ensure_phoenix_server.py` — CLI script to manage a singleton Phoenix server lifecycle. Supports `--mode submit` (screen session on submit node) and `--mode slurm` (SLURM job). Uses `flock` for concurrent invocation safety. Called by `exec_apptainer_harmonia.sh` before experiment launch.
  - New dataclass: `TracingConfig` in `config.py` — `enabled: bool`, `phoenix_endpoint: str`. Parsed from `tracing:` YAML section. Added to `ExperimentConfig`.
  - New field: `ExperimentConfig.config_source_path` — set by `load_config()` to the resolved path of the source YAML file. Enables automatic config snapshot copying to results directories.
  - `TurnRecord` enriched with: `input_tokens`, `output_tokens`, `cost_usd`, `code_executions`, `usage_records` (all backward-compatible with zero/empty defaults).
  - `ExperimentTrace` enriched with: `config_snapshot: Optional[dict]` (serialized in `to_dict()` output).
  - `TraceLogger.log_turn()` updated to accept and store new token/cost/code_execution fields.
  - `ExperimentRunner` (`runner.py`): wraps experiment in root `experiment_span()`, each turn in `turn_span()` with child `llm_call_span()` and `tool_span()` spans. Extracts `usage_records` from WebSocket messages. Calculates per-turn token costs using `ModelMetadataConfig` pricing. Copies config YAML to results dir as `config_snapshot.yaml`. Creates retry spans with error attributes.
  - `ManualExperimentRunner` (`manual_runner.py`): same span wrapping for passively monitored sessions. Root span uses manual context manager lifecycle (open on start, close on finalize).
  - `exec_apptainer_harmonia.sh`: new block after results dir creation checks `tracing.enabled` in config YAML and calls `ensure_phoenix_server.py`, exporting `PHOENIX_ENDPOINT` env var.
  - `.gitignore`: added `.phoenix/` entry.
  - `__init__.py`: exports `TracingConfig`, `init_tracing`, `extract_usage_records`, `extract_code_executions`, `calculate_turn_cost`.

- **Token usage forwarding (Archytas + Beaker changes — requires Apptainer rebuild):**
...
- **OTel Span Schema (OpenInference conventions):**
  - Root: `AGENT` span `experiment:{name}` with `harmonia.run_id`, `harmonia.trace_type`, `llm.model_name`, `harmonia.config_snapshot`.
  - Turn: `CHAIN` span `turn:{N}` with `harmonia.turn_number`, `input.value`, `output.value`.
  - LLM call: `LLM` span `llm_call:{N}` with `llm.token_count.prompt/completion/total`, `harmonia.cost_usd`.
  - Tool: `TOOL` span `beaker_execute` with `tool.name`, `input.value`, `output.value`, `tool.status`.
  - Retry: `LLM` span with `harmonia.error_code`, `harmonia.retry_attempt`, status=ERROR.
  - Custom namespace: all Harmonia attributes use `harmonia.*` prefix.
...
- **Consecutive tool error guardrail (P1 feature):**
  ...
  - New config field: `ArchytasContextConfig.max_consecutive_tool_errors` (default 3).
  - New env var: `ARCHYTAS_MAX_CONSECUTIVE_TOOL_ERRORS` (generated by `generate_env.py`, read by `bdikit_context/context.py` and `code_context/context.py`).
  - All 38 experiment YAML configs updated with `max_consecutive_tool_errors: 3`.
  - Requires Apptainer image rebuild to take effect (Archytas is installed from source at build time).

- **Python interpreter fix (P2 bugfix):**
  - `exec_apptainer_harmonia.sh` now invokes `ollama_launcher.py` via `"${SCRIPT_DIR}/.venv/bin/python"` instead of bare `python3`, which resolved to system Python 3.6.8 (incompatible with `from __future__ import annotations`). Affects port calculation (line 37) and VRAM estimation (line 669).
...
- **Dead code cleanup and liteLLM migration (WP-A):**
  - Deleted `litellm_direct.py` (direct LLM runner without Beaker) -- no longer needed.
  - Deleted diagnostic/test scripts: `check_archytas.py`, `diagnose_llm.py`, `quick_test.py`, `test_anyllm_adapter.py`, `test_anyllm_basic.py`.
  - Moved `diagnose_interactive_beaker_session.py` to `code_development_tools_agents/monitoring_and_evaluation/`.
  - Removed `_set_context_ws()` method from `src/automation/client.py`.
  - Removed `use_anyllm` field and `get_effective_provider()` from `src/bdikit_context/config/__init__.py`.
  - Removed 14 explicit `anyllm:*` entries from `PROVIDER_IMPORT_MAP` (the runtime fallback still handles `anyllm:` prefixes transparently).
  - All YAML configs and job scripts migrated from `anyllm:` prefix to direct provider names (e.g., `anyllm:openrouter` to `openrouter`).
  - `kernel_state_budget.py` got a REFERENCE ONLY header comment.
  - All three context files got clarifying comments on `apply_openrouter_hardening()`.

- **ArchytasContextConfig wiring (WP-B):**
  - `max_react_steps`, `max_errors`, and `max_consecutive_tool_errors` now wired via env vars (`ARCHYTAS_MAX_REACT_STEPS`, `ARCHYTAS_MAX_ERRORS`, `ARCHYTAS_MAX_CONSECUTIVE_TOOL_ERRORS`) in `bdikit_context/context.py` and `code_context/context.py`.
  - Not wired for `codeact_context` since it uses its own agent loop.

- **Provider prefix consolidation (WP-C):**
  - New file: `src/bdikit_context/llm/provider_prefixes.py` -- shared `LITELLM_PROVIDER_PREFIX` dict (single source of truth for 15 provider-to-litellm-prefix mappings).
  - `litellm_model.py`, `agent.py`, and `codeact_context/context.py` now import from the shared prefix table instead of maintaining inline/local dicts.
  - `generate_jobs.py` overhauled: defaults updated (2h time, 20G mem, 50G tmpspace), auto-GPU detection from provider name, associated .env auto-detection, generation header in output scripts.

- **Runner config and LLMConfig rename (WP-E):**
  - `LLMConfig` renamed to `ContainerLLMConfig` in `src/bdikit_context/config/__init__.py`.
  - `OutputConfig` in `src/automation/config.py` now has `input_files: list[str]` field (configurable per experiment).
  - `runner.py` uses `self.config.output.input_files` instead of hardcoded list.
  - 12 generation-1 automated configs: removed `results/` prefix from artifact paths in messages.
  - 9 configs that were missing `context: bdikit_context` got it added.

- **Ollama Python launcher (WP-F):**
  - New file: `src/automation/ollama_launcher.py` -- Python-based VRAM estimation and dynamic port calculation.
  - `exec_apptainer_harmonia.sh` now calls Python for VRAM estimation and port calculation (with bash fallback).
  - New tests: `tests/test_ollama_launcher.py` (13 tests).

- **Quality tooling (WP-G):**
  - New file: `.pre-commit-config.yaml` with ruff, shellcheck, yamllint hooks.
  - Ruff config added to `pyproject.toml` (`[tool.ruff]`, `[tool.ruff.lint]`, `[tool.pytest.ini_options]`).
  - New tests: `tests/test_config_loading.py` (31 parametrized tests), `tests/test_metrics_smoke.py` (2 tests).
  - New fixture: `tests/fixtures/sample_metrics.json`.

- **Entry point registration:**
  - `codeact_context` entry point registered in `pyproject.toml` alongside `bdikit_context` and `code_context`.
...
- **Implemented:** Shared provider prefix mapping (`src/bdikit_context/llm/provider_prefixes.py`) — single source of truth for 15 provider-to-litellm-prefix mappings, consumed by `litellm_model.py`, `agent.py`, and `codeact_context/context.py`.
- **Implemented:** Python Ollama launcher (`src/automation/ollama_launcher.py`) — VRAM estimation and dynamic port calculation called from `exec_apptainer_harmonia.sh` with bash fallback.
- **Implemented:** Pre-commit hooks (`.pre-commit-config.yaml`) — ruff linter/formatter, shellcheck, yamllint.
- **Implemented:** Test suite (`tests/`) — 46 tests covering config loading, metrics, and Ollama launcher.
- **Implemented:** Config management CLI (`manage_configs.py`) — list, get, set, clone, regenerate, validate experiment configs.
...
sleep 10
python run_experiment.py --config configs/automated/dou_harmonization_devstral.yaml
```
...
python /hpc/compgen/projects/llm_GEO_project/harmonia_metadata_agent/analysis/dstoker/harmonia/run_experiment.py \
    --config /hpc/compgen/projects/llm_GEO_project/harmonia_metadata_agent/analysis/dstoker/harmonia/experiments/experiment_1_harmonia_dou2020_gdc/configs/automated/dou_harmonization_devstral.yaml
kill $BEAKER_PID
'
```
...
│   │   └── ollama_launcher.py # Python VRAM estimation and dynamic port calculation for Ollama
│   │
...
│   │   │   └── __init__.py   # ContainerLLMConfig, HarmoniaConfig, get_config(), reset_config()
...
│   │   │   └── provider_prefixes.py  # LITELLM_PROVIDER_PREFIX dict (single source of truth for 15 provider mappings)
...
│       ├── diagnose_interactive_beaker_session.py   # Interactive Beaker session diagnostics (moved from root)
...
│
├── tests/                    # Test suite
│   ├── test_config_loading.py    # 31 parametrized config loading tests
│   ├── test_metrics_smoke.py     # 2 metrics smoke tests
│   ├── test_ollama_launcher.py   # 13 Ollama launcher tests
│   └── fixtures/
│       └── sample_metrics.json   # Test fixture for metrics tests
│
...
├── generate_jobs.py          # Script to generate SLURM job scripts (2h/20G/50G defaults, auto-GPU detection)
...
└── .pre-commit-config.yaml   # Pre-commit hooks: ruff, shellcheck, yamllint
```
...
Use this as a fallback if the new image has issues, but note that `litellm:*` providers won't work.
...
class OutputConfig:
    ...
    input_files: list[str] = field(default_factory=list)  # Configurable input files to copy into results dir
...
class ArchytasContextConfig:
    ...
    max_errors: int = 3
    max_consecutive_tool_errors: Optional[int] = 3  # Consecutive failures of same tool before FailedTaskError
...
class BDIKitContext(BeakerContext):
    ...
    def __init__(self, beaker_kernel: "BeakerKernel", config: Dict[str, Any]) -> None
        ...
        # Reads ARCHYTAS_MAX_REACT_STEPS / ARCHYTAS_MAX_ERRORS / ARCHYTAS_MAX_CONSECUTIVE_TOOL_ERRORS env vars → wires into agent kwargs
...
**Class: `ContainerLLMConfig`** (for bdikit_context, renamed from `LLMConfig` in WP-E)
```python
@dataclass
class ContainerLLMConfig:
    provider: str = "openai"  # Can be "litellm:openai", "litellm:ollama", "openrouter", etc.
    model: str = "gpt-4o"
...
PROVIDER_IMPORT_MAP = {
    ...
    ...
    # NOTE: Explicit anyllm:* entries were removed in WP-A (03-03-2026).
    # The runtime fallback logic still handles anyllm: prefixes transparently
    # by stripping the prefix and resolving to the underlying provider.
}
...
class ChatLiteLLM:
    ...
    def _build_model_string(self) -> str  # Maps provider+model to litellm format via LITELLM_PROVIDER_PREFIX from provider_prefixes.py
...
class LiteLLMModel(BaseArchytasModel):
    ...
    def auth(self, **kwargs) -> None          # Handles litellm: prefix stripping (anyllm: handled at runtime fallback level)
...
#### `llm/provider_prefixes.py` - Provider Prefix Mapping (Single Source of Truth)

Shared dictionary mapping Harmonia provider names to litellm model-string prefixes. All modules that need provider-to-prefix mapping import from here instead of maintaining local dicts.
...
**Consumers:** `litellm_model.py` (`ChatLiteLLM._build_model_string()`), `agent.py` (model normalization for bdi-kit calls), `codeact_context/context.py` (litellm model string construction).
...
llm:
  provider: ollama           # Direct provider name (or "litellm:ollama", "openrouter", etc.)
...
context_management:
  ...
  archytas:
    ...
    max_consecutive_tool_errors: 3    # Consecutive same-tool failures before abort
...
```yaml
experiment:
  name: "dou_gdc_manual_ollama_devstral"
...
All three contexts are registered via Python entry points in `pyproject.toml`:
```toml
...
**Backwards-compatible `anyllm:` prefix (deprecated):**
The `anyllm:*` provider strings still work at runtime via fallback logic, but explicit `PROVIDER_IMPORT_MAP` entries have been removed. All new configs use direct provider names (e.g., `ollama`, `openrouter`) or `litellm:` prefixed names.

### Environment Variables

```bash
# Provider and model
LLM_SERVICE_PROVIDER=ollama              # Direct provider name (preferred for new configs)
LLM_SERVICE_MODEL=devstral:latest
...
export LLM_SERVICE_PROVIDER="ollama"
...
python run_experiment.py experiments/experiment_1_harmonia_dou2020_gdc/configs/automated/dou_harmonization_devstral.yaml
```
...
## Ollama Python Launcher (`src/automation/ollama_launcher.py`)

Python module for VRAM estimation and dynamic port calculation for Ollama, used by `exec_apptainer_harmonia.sh` with a bash fallback.

**Functions:**

```python
def estimate_vram_mb(model_name: str, context_length: int = 4096, quantization: str = "Q4_K_M") -> dict
    # Returns dict with: model_params_b, quantization, context_length,
    # estimated_model_mb, estimated_kv_cache_mb, estimated_total_mb

def calculate_ollama_port(slurm_job_id: Optional[int] = None) -> int
    # Returns unique port: 11434 + 1 + (job_id % 200) for SLURM jobs, 11434 otherwise
```

**Integration:** `exec_apptainer_harmonia.sh` calls `python -c "from src.automation.ollama_launcher import ..."` for VRAM estimation and port calculation. If Python fails (e.g., outside venv), the script falls back to its original bash implementations.

**Tests:** 13 tests in `tests/test_ollama_launcher.py` covering VRAM estimation accuracy, port calculation edge cases, and SLURM job ID handling.
...
| Hook | Purpose |
|------|---------|
| `ruff` (linter) | Python linting with project-specific rules from `pyproject.toml` |
| `ruff` (formatter) | Python formatting (consistent style) |
| `shellcheck` | Shell script linting for bash scripts |
| `yamllint` | YAML syntax and style checking |

### Ruff Configuration (`pyproject.toml`)

Ruff linting and formatting rules are defined in `pyproject.toml`:
...
|-----------|-------|----------|
| `test_config_loading.py` | 31 parametrized | Config parsing, field validation, defaults for all config types |
| `test_metrics_smoke.py` | 2 | Metrics schema validation, sample metrics file loading |
| `test_ollama_launcher.py` | 13 | VRAM estimation, port calculation, SLURM integration |
