...
class BDIKitContext(BeakerContext):
    ...
    def _log_prompt_config(self) -> None  # Prints JSON diagnostic of prompt config
    async def setup(self, context_info=None, parent_header=None) -> None
    async def auto_context(self) -> str  # Returns system prompt (logs full prompt on first call)
```
