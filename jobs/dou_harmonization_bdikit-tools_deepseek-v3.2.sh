...
#SBATCH --job-name=harmonia_dou_harmonization_bdikit-tools_deepseek-v3.2
...
export RUN_ID
RUN_RESULTS_DIR="/hpc/compgen/projects/llm_GEO_project/harmonia_metadata_agent/analysis/dstoker/harmonia/results/dou_harmonization_bdikit-tools_deepseek-v3.2_${SLURM_JOB_ID}_${RUN_ID}"

# Redirect all output to date-stamped log files (includes run_id)
LOG_TIMESTAMP=$(date +%d-%m-%Y_%H%M)
LOG_OUT="logs/${LOG_TIMESTAMP}_dou_harmonization_bdikit-tools_deepseek-v3.2_${SLURM_JOB_ID}_${RUN_ID}.out"
LOG_ERR="logs/${LOG_TIMESTAMP}_dou_harmonization_bdikit-tools_deepseek-v3.2_${SLURM_JOB_ID}_${RUN_ID}.err"
mkdir -p logs
exec > "$LOG_OUT" 2> "$LOG_ERR"

echo "=============================================="
echo "Harmonia Experiment: dou_harmonization_bdikit-tools_deepseek-v3.2"
echo "=============================================="
echo ""
...
echo "LLM Model: deepseek/deepseek-v3.2"
echo ""

# Start Beaker server via exec script (handles Ollama automatically if needed)
./exec_apptainer_harmonia.sh \
    --port $PORT \
    --config experiments/experiment_1_harmonia_dou2020_gdc/configs/automated/dou_harmonization_bdikit-tools_deepseek-v3.2.yaml \
    --job-name "dou_harmonization_bdikit-tools_deepseek-v3.2_${SLURM_JOB_ID}" \
...
echo "Config: experiments/experiment_1_harmonia_dou2020_gdc/configs/automated/dou_harmonization_bdikit-tools_deepseek-v3.2.yaml"
echo ""
