...
echo "Pre-loading model olmo-3:latest..."
curl -s http://localhost:11434/api/generate -d "{\"model\": \"olmo-3:latest\", \"prompt\": \"Hello\", \"stream\": false}" > /dev/null 2>&1 || echo "  Warning: Model pre-load may have failed"
...
apptainer exec \
    ...
    --env LLM_SERVICE_MODEL=olmo-3:latest \
