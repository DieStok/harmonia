...
The original LLM abstraction used the `any-llm` Python package (`anyllm.py` module). Around commit `5806404`/`407536a`, this was replaced by `litellm`. The migration plan is documented in `docs/plans/25_02_2026_1530_change_LLM_backend_to_liteLLM.md`.
...
There is no automated way to track which experiments have been submitted, which are running, which succeeded or failed, and which need to be retried. The `docs/processes/02_03_2026_priority_run_ledger.csv` and `analysis/errors_02_03_2026_run_matrix.csv` are manually maintained spreadsheets. A simple DB or structured ledger tool that integrates with SLURM `squeue`/`sacct` outputs would dramatically reduce the manual triage burden.
