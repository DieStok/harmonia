...
**Date:** 02-04-2026 | **Python:** 3.11 via `.venv/` (never conda/system)
...
│   │   ├── make_standard_evaluation_plots.py  # Generate all standard plots (incl. grouped model bar charts)
│   │   └── visualization/       # io.py, enrich.py, normalize.py, aggregate.py, plots.py, report.py
│   │
│   ├── dashboard/               # Plotly Dash web dashboard
│   │   ├── app.py               # Main entry point + all callbacks (non-blocking Phoenix init)
...
│   ├── openrouter_hardening.py  # OpenRouter-specific resilience and error handling
...
│   ├── create_index_csvs.py     # Generate index CSVs from experiment results
│   └── update_config_yamls.py   # Batch-update experiment config YAML files
│
├── code_development_tools_agents/monitoring_and_evaluation/
│   ├── read_and_analyze_logs_and_traces_cli.py  # Log/trace failure analysis (20-class taxonomy)
│   └── types_of_log_and_trace_problems.yaml     # Machine-readable error taxonomy (v1.2)
│
├── LLM_associated_metadata/     # Model registries (OpenRouter + Ollama) + lookup tools
├── tests/                       # 87 tests: config loading, metrics, ollama launcher, prompt validation
│
...
Plotly Dash web app combining Phoenix traces, metrics.json, and trace.json. Phoenix initialization is non-blocking — dashboard starts immediately and loads Phoenix data asynchronously.
...
**Data loader:** `DashboardDataLoader` scans `results/` directories AND queries Phoenix. Outer-joins on `run_id`. Thread-safe. Degrades gracefully without Phoenix (shows local data only). Null-safe model labels throughout.
...
**Config management:** `manage_configs.py` (list, get, set, clone, validate). `clone` auto-enriches with `model_metadata` from registry. Batch updates via `scripts/update_config_yamls.py`.
...
**OpenRouter hardening:** `src/openrouter_hardening.py` provides resilience for OpenRouter-specific error conditions.
...
# Standard plots (includes grouped model bar charts)
...
**Index generation:** `scripts/create_index_csvs.py` generates index CSVs from experiment results for analysis.
...
20-class failure taxonomy (v1.2) across 6 categories: Infrastructure (1A hung server, 1B 405, 1C ZMQ), Model Config (2A not found, 2B no tools, 2C crash, 2D unavailable, 2E rate limit, 2F pre-load timeout), LLM Behavioral (3A timeout, 3B not using tools, 3C hallucinated output, 3D websocket size, 3E context exhaustion, 3F truncated, 3G silent empty, 3H ollama context truncation), Data/Config (4A file not found), Output (5A no output), Diagnostic (6A kernel state budget enforced).
...
- **Tests:** `tests/` — 87 tests (config loading, metrics, ollama launcher, prompt validation). Run: `.venv/bin/python -m pytest tests/ -v`
