...
**Date:** 03-03-2026 (updated from 02-03-2026)
...
- **ArchytasContextConfig wiring (WP-B):**
  - `max_react_steps` and `max_errors` now wired via env vars (`ARCHYTAS_MAX_REACT_STEPS`, `ARCHYTAS_MAX_ERRORS`) in `bdikit_context/context.py` and `code_context/context.py`.
...
class BDIKitContext(BeakerContext):
    ...
    def __init__(self, beaker_kernel: "BeakerKernel", config: Dict[str, Any]) -> None
        ...
        # Reads ARCHYTAS_MAX_REACT_STEPS / ARCHYTAS_MAX_ERRORS env vars → wires into agent kwargs
